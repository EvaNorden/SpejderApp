...
public class GameEndpoint {
    ...
    public Game insert(Game game) {
        ...
        System.out.println("Before Game with ID: " + game.getId());
    ...
    /**
     * This deletes an existing <code>Game</code> object.
     * @param id The id of the object to be deleted.
     */
    @ApiMethod(name = "removeGame")
    public void removeGame(@Named("id") Long id) throws NotFoundException {
        Game record = findRecord(id);
        if(record == null) {
            throw new NotFoundException("Game Record does not exist");
        }
        ofy().delete().entity(record).now();
    }

    //Private method to retrieve a <code>Quote</code> record
    private Game findRecord(Long id) {
